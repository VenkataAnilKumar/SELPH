...
class TwinEngineService:
    ...
    @staticmethod
    def _generate_draft_via_llm(system_prompt: str, user_prompt: str) -> tuple[str, dict, str, list]:
        ...
        return draft, normalized, reasoning, flags
    ...
    @staticmethod
    def _generate_draft(ctx: TwinContext, system_prompt: str, user_prompt: str) -> tuple[str, dict, str, list, str]:
        ...
        if settings.feature_twin_llm_drafts:
            try:
                llm_draft, llm_factors, llm_reasoning, llm_flags = TwinEngineService._generate_draft_via_llm(
                    system_prompt,
                    user_prompt,
                )
                llm_draft = TwinEngineService._apply_word_limit(llm_draft, ctx.constraints.max_words)
                return llm_draft, llm_factors, llm_reasoning, llm_flags, "llm"
    ...
    def run_twin_pipeline(db: Session, message_id: str, user_id: str) -> dict:
        ...
        draft, factors, generator_reasoning, generator_flags, generation_source = TwinEngineService._generate_draft(
            ctx,
            system_prompt,
            user_prompt,
        )
